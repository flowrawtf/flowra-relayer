//! Staging check for per-validator PBP enforcement on the block engine's delivery path.
//!
//! Two validator identities subscribe to the engine's packet stream and one pushes a blocking
//! policy. Packets are injected into the relayer's TPU, travel relayer -> engine -> validator,
//! and what each identity receives is compared.
//!
//! The engine serves every validator off one broadcast, so a policy applied at ingest would hit
//! all of them. This checks the drop happens on delivery instead, where the recipient is known.
...
use jito_protos::{
    block_engine::{
        block_engine_validator_client::BlockEngineValidatorClient, SubscribePacketsRequest,
    },
    auth::{
...
#[derive(Parser, Debug)]
#[command(about = "Verify per-validator PBP enforcement on the block engine")]
struct Args {
    /// Engine validator-service gRPC URL.
    #[arg(long)]
    url: String,
...
type AuthedEngine = BlockEngineValidatorClient<
...
async fn connect(url: &str, kp: &Keypair) -> Result<AuthedEngine, Box<dyn std::error::Error>> {
    ...
    Ok(BlockEngineValidatorClient::with_interceptor(channel, interceptor))
}

/// Count packets arriving on a subscription until `secs` elapse.
async fn collect(mut client: AuthedEngine, secs: u64, label: &'static str) -> Arc<Mutex<u64>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let digest = strict
        ...
        .into_inner()
        .policy_digest;
    println!("[+] strict pushed policy, engine digest = {digest}");
    if digest.is_empty() {
        return Err("engine returned an empty digest; it is not storing the policy".into());
    }
    ...
    if p == 0 {
        return Err("permissive validator received nothing; nothing reached the engine, so this run \
                    proves nothing about policy"
            .into());
    }
    if s >= p {
        return Err(format!(
