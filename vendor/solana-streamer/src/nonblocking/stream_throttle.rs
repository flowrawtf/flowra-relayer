...
///
/// FLOWRA PATCH (vendored, upstream 4.2.1 value: 200): this is the fixed per-connection
/// allowance for unstaked senders — 20 streams per 100ms window at 200 TPS — and on a
/// dedicated relayer box it, not connection count, is what throttles heavy senders during
/// the fronted validator's leader windows (observed 8k-24k throttled streams per window at
/// exactly the burst peak). 500 TPS = 50 streams/100ms per connection. The staked floor
/// (`max_unstaked + 1`) rises with it, which only helps. The validator's own defaults are
/// sized for a box that also runs consensus; this one does nothing else.
